...
export async function fetchMerklRewards(
  ...
  // Merkl v4 response is an array of reward entries per campaign/token
  // Each entry has: { token, symbol, amount, claimed, proofs }
  if (Array.isArray(data)) {
    for (const entry of data) {
      // Each chain entry can have nested reward data
      const chainData = entry?.rewards ?? entry;
      if (!chainData) continue;

      // Walk the rewards structure — Merkl v4 nests by campaign
